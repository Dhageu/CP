...
figure::figure(float x1, float x2, float x3, float x4, float y1, float y2, float y3, float y4) {
	...
	this->y4 = y4;
	float AB, BC, CD, AD, p;
	AB = sqrt(((this->x2 - this->x1) * (this->x2 - this->x1)) + ((this->y2 - this->y1) * (this->y2 - this->y1)));
	BC = sqrt(((this->x3 - this->x2) * (this->x3 - this->x2)) + ((this->y3 - this->y2) * (this->y3 - this->y2)));
	CD = sqrt(((this->x4 - this->x3) * (this->x4 - this->x3)) + ((this->y4 - this->y3) * (this->y4 - this->y3)));
	AD = sqrt(((this->x4 - this->x1) * (this->x4 - this->x1)) + ((this->y4 - this->y1) * (this->y4 - this->y1)));
	p = (AB + BC + CD + AD) / 2;
	this->S = sqrt((p - AB) * (p - BC) * (p - CD) * (p - AD));
	this->P = AB + BC + CD + AD;
}
bool figure::is_prug() {
...
void figure::show() {
	...
	cout << "Площадь четырехугольника ABCD = " << S << endl;
	cout << "Периметр четырехугольника ABCD = " << P << endl << endl;
